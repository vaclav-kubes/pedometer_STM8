 ...
 820  00f6 72125280      	bset	21120,#1
 822  00fa 2004          	jra	L713
 823  00fc               L513:
 824                     ; 408     TIM3->CR1 &= (uint8_t)(~TIM_CR1_UDIS);
 826  00fc 72135280      	bres	21120,#1
 827  0100               L713:
 828                     ; 410 }
 831  0100 81            	ret
 ...
 889                     ; 421 {
 890                     	switch	.text
 891  0101               _TIM3_UpdateRequestConfig:
 ...
 903  0105 72145280      	bset	21120,#2
 905  0109 2004          	jra	L153
 906  010b               L743:
 907                     ; 432     TIM3->CR1 &= (uint8_t)(~TIM_CR1_URS);
 909  010b 72155280      	bres	21120,#2
 910  010f               L153:
 911                     ; 434 }
 914  010f 81            	ret
 ...
 950                     ; 443 {
 951                     	switch	.text
 952  0110               _TIM3_ARRPreloadConfig:
 ...
 964  0113 721e5280      	bset	21120,#7
 966  0117 2004          	jra	L373
 967  0119               L173:
 968                     ; 454     TIM3->CR1 &= (uint8_t)(~TIM_CR1_ARPE);
 970  0119 721f5280      	bres	21120,#7
 971  011d               L373:
 972                     ; 456 }
 975  011d 81            	ret
...
1032                     ; 467 {
1033                     	switch	.text
1034  011e               _TIM3_SelectOnePulseMode:
...
1046  0122 72165280      	bset	21120,#3
1048  0126 2004          	jra	L524
1049  0128               L324:
1050                     ; 478     TIM3->CR1 &= (uint8_t)(~TIM_CR1_OPM);
1052  0128 72175280      	bres	21120,#3
1053  012c               L524:
1054                     ; 480 }
1057  012c 81            	ret
...
1106  0130 72105280      	bset	21120,#0
1108  0134 2004          	jra	L744
1109  0136               L544:
1110                     ; 500     TIM3->CR1 &= (uint8_t)(~TIM_CR1_CEN);
1112  0136 72115280      	bres	21120,#0
1113  013a               L744:
1114                     ; 502 }
1117  013a 81            	ret
...
1866                     ; 784 {
1867                     	switch	.text
1868  01fe               _TIM3_CtrlPWMOutputs:
...
1880  0201 721e5295      	bset	21141,#7
1882  0205 2004          	jra	L7001
1883  0207               L5001:
1884                     ; 796     TIM3->BKR &= (uint8_t)(~TIM_BKR_MOE) ;
1886  0207 721f5295      	bres	21141,#7
1887  020b               L7001:
1888                     ; 798 }
1891  020b 81            	ret
...
2259                     ; 910 {
2260                     	switch	.text
2261  0268               _TIM3_ForcedOC2Config:
2263  0268 88            	push	a
2264  0269 88            	push	a
...
2333                     ; 934 {
2334                     	switch	.text
2335  0282               _TIM3_OC1PreloadConfig:
...
2394                     ; 956 {
2395                     	switch	.text
2396  0290               _TIM3_OC2PreloadConfig:
...
2575                     ; 1025 {
2576                     	switch	.text
2577  02ba               _TIM3_OC1PolarityConfig:
...
2636                     ; 1049 {
2637                     	switch	.text
2638  02c9               _TIM3_OC2PolarityConfig:
...
2950  0311 cd03e3        	call	_TIM3_SetIC1Prescaler
...
2969  0325 cd03fd        	call	_TIM3_SetIC2Prescaler
2971  0328               L7641:
2972                     ; 1209 }
2975  0328 85            	popw	x
2976  0329 81            	ret
...
3142  035b cd03e3        	call	_TIM3_SetIC1Prescaler
...
3158  036d cd03fd        	call	_TIM3_SetIC2Prescaler
...
3177  0381 ad7a          	call	_TIM3_SetIC2Prescaler
...
3193  0392 ad4f          	call	_TIM3_SetIC1Prescaler
3195  0394               L1551:
3196                     ; 1297 }
...
3443                     ; 1348 {
3444                     	switch	.text
3445  03e3               _TIM3_SetIC1Prescaler:
3447  03e3 88            	push	a
3448  03e4 88            	push	a
...
3526                     ; 1376 {
3527                     	switch	.text
3528  03fd               _TIM3_SetIC2Prescaler:
3530  03fd 88            	push	a
3531  03fe 88            	push	a
...
3670  041d ca5285        	or	a,21125
3671  0420 c75285        	ld	21125,a
3673  0423 2009          	jra	L5371
3674  0425               L3371:
3675                     ; 1436     TIM3->IER &= (uint8_t)(~(uint8_t)TIM3_IT);
3677  0425 7b01          	ld	a,(OFST+1,sp)
3678  0427 43            	cpl	a
3679  0428 c45285        	and	a,21125
3680  042b c75285        	ld	21125,a
3681  042e               L5371:
3682                     ; 1438 }
3685  042e 85            	popw	x
3686  042f 81            	ret
...
4295  0495 ca5284        	or	a,21124
4296  0498 c75284        	ld	21124,a
4298  049b 2009          	jra	L3122
4299  049d               L1122:
4300                     ; 1595     TIM3->DER &= (uint8_t)(~TIM3_DMASource);
4302  049d 7b01          	ld	a,(OFST+1,sp)
4303  049f 43            	cpl	a
4304  04a0 c45284        	and	a,21124
4305  04a3 c75284        	ld	21124,a
4306  04a6               L3122:
4307                     ; 1597 }
4310  04a6 85            	popw	x
4311  04a7 81            	ret
...
4395                     ; 1645 {
4396                     	switch	.text
4397  04b6               _TIM3_InternalClockConfig:
4401                     ; 1647   TIM3->SMCR &=  (uint8_t)(~TIM_SMCR_SMS);
4403  04b6 c65282        	ld	a,21122
4404  04b9 a4f8          	and	a,#248
4405  04bb c75282        	ld	21122,a
4406                     ; 1648 }
4409  04be 81            	ret
...
4498                     ; 1668 {
4499                     	switch	.text
4500  04bf               _TIM3_TIxExternalClockConfig:
...
4544  04e2 ad4b          	call	_TIM3_SelectInputTrigger
4546                     ; 1688   TIM3->SMCR |= (uint8_t)(TIM3_SlaveMode_External1);
4548  04e4 c65282        	ld	a,21122
4549  04e7 aa07          	or	a,#7
4550  04e9 c75282        	ld	21122,a
4551                     ; 1689 }
4554  04ec 85            	popw	x
4555  04ed 81            	ret
...
4672                     ; 1710 {
4673                     	switch	.text
4674  04ee               _TIM3_ETRClockMode1Config:
...
4773                     ; 1742 {
4774                     	switch	.text
4775  051d               _TIM3_ETRClockMode2Config:
...
4789  0526 ad63          	call	_TIM3_ETRConfig
4791  0528 84            	pop	a
4792                     ; 1747   TIM3->ETR |= TIM_ETR_ECE ;
4794  0529 721c5283      	bset	21123,#6
4795                     ; 1748 }
4798  052d 85            	popw	x
4799  052e 81            	ret
...
4915                     ; 1800 {
4916                     	switch	.text
4917  052f               _TIM3_SelectInputTrigger:
4919  052f 88            	push	a
4920  0530 88            	push	a
...
5052                     ; 1828 {
5053                     	switch	.text
5054  0549               _TIM3_SelectOutputTrigger:
5056  0549 88            	push	a
5057  054a 88            	push	a
...
5173                     ; 1856 {
5174                     	switch	.text
5175  0563               _TIM3_SelectSlaveMode:
5177  0563 88            	push	a
5178  0564 88            	push	a
...
5247                     ; 1880 {
5248                     	switch	.text
5249  057d               _TIM3_SelectMasterSlaveMode:
...
5347  0591 ca5283        	or	a,21123
5348  0594 c75283        	ld	21123,a
5349                     ; 1923 }
5352  0597 85            	popw	x
5353  0598 81            	ret
...
5466                     ; 1961 {
5467                     	switch	.text
5468  0599               _TIM3_EncoderInterfaceConfig:
...
5620                     ; 2016 {
5621                     	switch	.text
5622  05ff               _TIM3_SelectHallSensor:
...
5746  0617 7211528b      	bres	21131,#0
