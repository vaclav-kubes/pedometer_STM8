...
1051                     ; 410       break;
1052  0226 4d            	tnz	a
1053  0227 271e          	jreq	L343
1054  0229 a002          	sub	a,#2
1055  022b 2723          	jreq	L543
1056  022d a002          	sub	a,#2
1057  022f 272a          	jreq	L743
1058  0231 a002          	sub	a,#2
1059  0233 2732          	jreq	L153
1060  0235 a00a          	sub	a,#10
1061  0237 273c          	jreq	L353
1062  0239 a002          	sub	a,#2
1063  023b 2741          	jreq	L553
1064  023d a002          	sub	a,#2
1065  023f 2748          	jreq	L753
1066  0241 a002          	sub	a,#2
1067  0243 2750          	jreq	L163
1068  0245 205a          	jra	L114
1069  0247               L343:
1070                     ; 385     case EXTI_Pin_0:
1070                     ; 386       value = (uint8_t)(EXTI->CR1 & EXTI_CR1_P0IS);
1072  0247 c650a0        	ld	a,20640
1073  024a a403          	and	a,#3
1074  024c 6b01          	ld	(OFST+0,sp),a
1076                     ; 387       break;
1078  024e 2051          	jra	L114
1079  0250               L543:
1080                     ; 388     case EXTI_Pin_1:
1080                     ; 389       value = (uint8_t)((uint8_t)(EXTI->CR1 & EXTI_CR1_P1IS) >> EXTI_Pin_1);
1082  0250 c650a0        	ld	a,20640
1083  0253 a40c          	and	a,#12
1084  0255 44            	srl	a
1085  0256 44            	srl	a
1086  0257 6b01          	ld	(OFST+0,sp),a
1088                     ; 390       break;
1090  0259 2046          	jra	L114
1091  025b               L743:
1092                     ; 391     case EXTI_Pin_2:
1092                     ; 392       value = (uint8_t)((uint8_t)(EXTI->CR1 & EXTI_CR1_P2IS) >> EXTI_Pin_2);
1094  025b c650a0        	ld	a,20640
1095  025e a430          	and	a,#48
1096  0260 4e            	swap	a
1097  0261 a40f          	and	a,#15
1098  0263 6b01          	ld	(OFST+0,sp),a
1100                     ; 393       break;
1102  0265 203a          	jra	L114
1103  0267               L153:
1104                     ; 394     case EXTI_Pin_3:
1104                     ; 395       value = (uint8_t)((uint8_t)(EXTI->CR1 & EXTI_CR1_P3IS) >> EXTI_Pin_3);
1106  0267 c650a0        	ld	a,20640
1107  026a a4c0          	and	a,#192
1108  026c 4e            	swap	a
1109  026d 44            	srl	a
1110  026e 44            	srl	a
1111  026f a403          	and	a,#3
1112  0271 6b01          	ld	(OFST+0,sp),a
1114                     ; 396       break;
1116  0273 202c          	jra	L114
1117  0275               L353:
1118                     ; 397     case EXTI_Pin_4:
1118                     ; 398       value = (uint8_t)(EXTI->CR2 & EXTI_CR2_P4IS);
1120  0275 c650a1        	ld	a,20641
1121  0278 a403          	and	a,#3
1122  027a 6b01          	ld	(OFST+0,sp),a
1124                     ; 399       break;
1126  027c 2023          	jra	L114
1127  027e               L553:
1128                     ; 400     case EXTI_Pin_5:
1128                     ; 401       value = (uint8_t)((uint8_t)(EXTI->CR2 & EXTI_CR2_P5IS) >> ((uint8_t)EXTI_Pin_5 & (uint8_t)0x0F));
1130  027e c650a1        	ld	a,20641
1131  0281 a40c          	and	a,#12
1132  0283 44            	srl	a
1133  0284 44            	srl	a
1134  0285 6b01          	ld	(OFST+0,sp),a
1136                     ; 402       break;
1138  0287 2018          	jra	L114
1139  0289               L753:
1140                     ; 403     case EXTI_Pin_6:
1140                     ; 404       value = (uint8_t)((uint8_t)(EXTI->CR2 & EXTI_CR2_P6IS) >> ((uint8_t)EXTI_Pin_6 & (uint8_t)0x0F));
1142  0289 c650a1        	ld	a,20641
1143  028c a430          	and	a,#48
1144  028e 4e            	swap	a
1145  028f a40f          	and	a,#15
1146  0291 6b01          	ld	(OFST+0,sp),a
1148                     ; 405       break;
1150  0293 200c          	jra	L114
1151  0295               L163:
1152                     ; 406     case EXTI_Pin_7:
1152                     ; 407       value = (uint8_t)((uint8_t)(EXTI->CR2 & EXTI_CR2_P7IS) >> ((uint8_t)EXTI_Pin_7 & (uint8_t)0x0F));
1154  0295 c650a1        	ld	a,20641
1155  0298 a4c0          	and	a,#192
1156  029a 4e            	swap	a
1157  029b 44            	srl	a
1158  029c 44            	srl	a
1159  029d a403          	and	a,#3
1160  029f 6b01          	ld	(OFST+0,sp),a
1162                     ; 408       break;
1164  02a1               L363:
1165                     ; 409     default:
1165                     ; 410       break;
...
1170  02a1 7b01          	ld	a,(OFST+0,sp)
1173  02a3 5b01          	addw	sp,#1
1174  02a5 81            	ret
1220                     ; 427 EXTI_Trigger_TypeDef EXTI_GetPortSensitivity(EXTI_Port_TypeDef EXTI_Port)
1220                     ; 428 {
1221                     	switch	.text
1222  02a6               _EXTI_GetPortSensitivity:
1224  02a6 88            	push	a
1225  02a7 88            	push	a
...
1235  02a8 a5f0          	bcp	a,#240
1236  02aa 2614          	jrne	L534
1237                     ; 438     portsensitivity = (uint8_t)((uint8_t)0x03 & (uint8_t)(EXTI->CR3 >> EXTI_Port));
1239  02ac 7b02          	ld	a,(OFST+1,sp)
1240  02ae 5f            	clrw	x
1241  02af 97            	ld	xl,a
1242  02b0 c650a2        	ld	a,20642
1243  02b3 5d            	tnzw	x
1244  02b4 2704          	jreq	L211
1245  02b6               L411:
1246  02b6 44            	srl	a
1247  02b7 5a            	decw	x
1248  02b8 26fc          	jrne	L411
1249  02ba               L211:
1250  02ba a403          	and	a,#3
1251  02bc 6b01          	ld	(OFST+0,sp),a
1254  02be 2014          	jra	L734
1255  02c0               L534:
1256                     ; 444     portsensitivity = (uint8_t)((uint8_t)0x03 & (uint8_t)(EXTI->CR4 >> (EXTI_Port & 0x0F)));
1258  02c0 7b02          	ld	a,(OFST+1,sp)
1259  02c2 a40f          	and	a,#15
1260  02c4 5f            	clrw	x
1261  02c5 97            	ld	xl,a
1262  02c6 c650aa        	ld	a,20650
1263  02c9 5d            	tnzw	x
1264  02ca 2704          	jreq	L611
1265  02cc               L021:
1266  02cc 44            	srl	a
1267  02cd 5a            	decw	x
1268  02ce 26fc          	jrne	L021
1269  02d0               L611:
1270  02d0 a403          	and	a,#3
1271  02d2 6b01          	ld	(OFST+0,sp),a
1273  02d4               L734:
1274                     ; 447   return((EXTI_Trigger_TypeDef)portsensitivity);
1276  02d4 7b01          	ld	a,(OFST+0,sp)
1279  02d6 85            	popw	x
1280  02d7 81            	ret
